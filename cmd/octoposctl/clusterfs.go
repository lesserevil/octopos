...
func clusterFSUnitContent(mountPoint, metaURL, options string) string {
	if options == "" {
...
ExecStart=/usr/local/bin/juicefs mount -o %s %s %s
...
`, mountPoint, options, metaURL, mountPoint, mountPoint, mountPoint)
}
...
func installLocalClusterFSService(mountPoint, metaURL, options string) error {
	...
	if err := os.WriteFile(tmpSvc, []byte(clusterFSUnitContent(mountPoint, metaURL, options)), 0644); err != nil {
